...
router.get('/epic/callback',
  ...
  (req, res) => {
    ...
    if (req.user.isBanned) {
      return res.redirect('/?error=banned&message=Your account has been banned');
    }
...
router.post('/logout', async (req, res) => {
  ...
  // Отзыв токена в Epic Games
  if (refreshToken) {
    try {
      await fetch('https://api.epicgames.dev/epic/oauth/v2/revoke', {
        method: 'POST',
        headers: {
      ...
      console.log(`✅ Token revoked for user ${nickname}`);
    } catch (error) {
      console.error('Failed to revoke Epic token:', error.message);
    }
  }
